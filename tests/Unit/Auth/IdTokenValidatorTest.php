...
beforeEach(function () {
    ...
    $this->validator = new IdTokenValidator(keyResolver([$this->provider->jwks('kid')]), 'client-123');
});
...
it('refetches the JWKS once for an unknown kid', function () {
    ...
    $validator = new IdTokenValidator(keyResolver([$old->jwks('old'), $new->jwks('new')], $history), 'client-123');
